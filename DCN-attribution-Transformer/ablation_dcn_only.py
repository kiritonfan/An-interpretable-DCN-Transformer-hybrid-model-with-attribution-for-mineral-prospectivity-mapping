"""This script runs the DCN-only ablation experiment for the study."""
...
GRAD_CLIP        = 1.0
DROPOUT          = 0.3                                        
MODEL_SAVE_PATH  = 'best_dcn_only_model.pth'
OUTPUT_DIR       = 'prediction_results_dcn_only'
...
def train(model, train_loader, val_loader, num_epochs=NUM_EPOCHS, class_weights=None):
    ...
    print(f"\n[DCN-only] Starting training for {num_epochs} epochs")
    ...
    _plot_history(history, save_path='dcn_only_training_history.png', title='DCN-only Model Training History')
...
class DCNOnlyPredictor:

    def __init__(self, model_path, feature_dim=FEATURE_DIM, device=None):
        self.device = device or torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        print(f"Device in Use: {self.device}")

        print("Loading the DCN-only model...")
        self.model, self.checkpoint = self._load_model(model_path, feature_dim)
        self.model.eval()
    ...
    def _load_model(self, model_path, feature_dim):
        model = DCNv2(num_classes=NUM_CLASSES, output_dim=feature_dim).to(self.device)
    ...
    def predict_batch(self, data, batch_size=96, show_progress=True):
        ...
        with torch.no_grad():
            ...
            if show_progress:
                itr = tqdm(itr, desc="DCN-Only Prediction progress", unit="batch")
    ...
    def predict_and_save(self, data, output_dir=OUTPUT_DIR,
        ...
        config = {
            'model_architecture': 'DCN-Only (Ablation)',
        ...
        _visualize_results(probabilities, output_dir, confidence_threshold, title_suffix='(DCN-only)')
...
def _print_statistics(probabilities, threshold=0.5):
    ...
    print("\n" + "="*55)
    print("Prediction Statistics - DCN-only Model")
    print("="*55)
...
def main():
    print("=" * 60)
    print("Ablation Study - DCN-only Model Mineral Prediction")
    print("=" * 60)
    ...
    for name, arr, expected in [
        ('Training data', train_data, (None, 9, 9, 42)),
        ('Validation data', val_data,   (None, 9, 9, 42))
    ]:
    ...
    print("\n=== Creating DCN-only model ===")
    model = DCNv2(num_classes=NUM_CLASSES, output_dim=FEATURE_DIM).to(device)

    total = sum(p.numel() for p in model.parameters())
    print(f"Model architecture: Block1(Conv 42→64) + Block2(DCNv2Block 64→128) + Block3(1x1Conv 128→{FEATURE_DIM})")
    print(f"Classifier: AvgPool → Linear({FEATURE_DIM},{max(64,FEATURE_DIM//2)}) → ReLU → Dropout(0.3) → Linear({max(64,FEATURE_DIM//2)},2)")
    print(f"Total parameters: {total:,}")
    ...
    model.load_state_dict(checkpoint['model_state_dict'])
    metrics = compute_val_metrics(model, val_loader, model_name='DCN-onlyModel')
    ...
    print("=" * 60)
    print("Ablation Study - DCN-only Model completed!")
    print("=" * 60)
    ...
    print(f"Training curve: dcn_only_training_history.png")
