"""This script runs the DCN-Transformer ablation experiment for the study."""
...
GRAD_CLIP        = 1.0
DROPOUT          = 0.15                                 
MODEL_SAVE_PATH  = 'best_dcn_transformer_model.pth'
OUTPUT_DIR       = 'prediction_results_dcn_transformer'
...
class DCNTransformerModel(nn.Module):
    def __init__(self, num_classes=2, feature_dim=FEATURE_DIM,
        ...
        super(DCNTransformerModel, self).__init__()
        ...
        print(f"[OK] DCN-TransformerModel（No Attribution）")
        print(f"  DCNv2: Block1(Conv 42→64) + Block2(DCNv2Block 64→128) + Block3(1x1Conv 128→{feature_dim})")
        ...
        print(f"  [Note] Model，removed attribution network  note")
    ...
    def forward(self, x):
        ...
        output = self.transformer(dcn_features)                                
        return output

    def forward_with_attention(self, x):
        dcn_features = self.dcn(x, extract_features=True)
        output, attention_maps = self.transformer(dcn_features, return_attention=True)
...
def train(model, train_loader, val_loader, num_epochs=NUM_EPOCHS, class_weights=None):
    ...
    print(f"\n[DCN-Transformer] Starting training for {num_epochs} epochs")
    ...
    _plot_history(history, save_path='dcn_transformer_training_history.png',
                  title='DCN-Transformer (No Attribution) Model Training History')
...
class DCNTransformerPredictor:
    ...
    def __init__(self, model_path, feature_dim=FEATURE_DIM,
        ...
        print("Loading the DCN-Transformer (No Attribution) model...")
    ...
    def predict_batch(self, data, batch_size=96, show_progress=True):
        ...
        with torch.no_grad():
            ...
            if show_progress:
                itr = tqdm(itr, desc="DCN-Transformer Prediction progress", unit="batch")
    ...
    def predict_and_save(self, data, output_dir=OUTPUT_DIR,
        ...
        config = {
            'model_architecture': 'DCN-Transformer (Ablation, No Attribution)',
            'feature_dim': FEATURE_DIM,
            'dcnv2_layers': 3,
            'transformer_depth': TRANSFORMER_DEPTH,
            'transformer_heads': TRANSFORMER_HEADS,
            ...
            'ablation_note': 'The attribution network and attribution-guided attention fusion are removed, and DCNv2 features are fed directly into DTransformer.'
        }
        ...
        _visualize_results(probabilities, output_dir, confidence_threshold,
                           title_suffix='(DCN-Transformer No Attribution)')
...
def _print_statistics(probabilities, threshold=0.5):
    ...
    print("\n" + "="*60)
    print("Prediction Statistics - DCN-Transformer (No Attribution) Model")
    print("="*60)
...
def main():
    print("=" * 60)
    print("Ablation Study - DCN-Transformer Model (No Attribution Guidance) Mineral Prediction")
    print("=" * 60)
    print("\nCompared with the full hybrid model:")
    print("  [Kept] DCNv2 feature extraction（Block1+Block2+Block3，3layer structure）")
    print("  [Kept] Transformer classification（Encoder + DTransformer）")
    print("  [Removed] attribution network (DotProductAttributionNetwork)")
    print("  [Removed] note (AttributionGuidedAttention)")
    ...
    print("\n=== Creating DCN-Transformer model (No Attribution) ===")
    model = DCNTransformerModel(
        num_classes=NUM_CLASSES,
        feature_dim=FEATURE_DIM,
        transformer_depth=TRANSFORMER_DEPTH,
    ...
    model.load_state_dict(checkpoint['model_state_dict'])
    metrics = compute_val_metrics(model, val_loader, model_name='DCN-TransformerModel（No Attribution）')
    ...
    predictor = DCNTransformerPredictor(
    ...
    print("=" * 60)
    print("Ablation Study - DCN-Transformer (No Attribution) completed!")
    print("=" * 60)
    ...
    print(f"Training curve: dcn_transformer_training_history.png")
