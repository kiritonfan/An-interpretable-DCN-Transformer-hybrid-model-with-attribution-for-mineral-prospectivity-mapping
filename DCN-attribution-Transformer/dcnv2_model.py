"""Implement the lightweight deformable convolutional feature extractor."""
...
class DCNv2Block(nn.Module):
    ...
    def forward(self, x):
        offset = self.offset_conv(x)  # (B, 2k²·g, H, W)
        mask = torch.sigmoid(self.mask_conv(x))  # (B, k²*g, H, W), range [0, 1]
        x = self.dcn(x, offset, mask)  # (B, out_ch, H, W)
        x = self.bn(x)
        x = self.relu(x)
        return x
...
class DCNv2(nn.Module):
    ...
    def forward(self, x, extract_features=False):
        x = self.block1(x)  # (B, 64,  9, 9)
        x = self.block2(x)  # (B, 128, 9, 9)
        features = self.block3(x)  # (B, output_dim, 9, 9)
